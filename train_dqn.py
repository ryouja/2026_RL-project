"""
DQN Agent Training
Learns to navigate chemical space toward target dipole range [2.0, 3.0] Debye
"""
...
def save_plots(rewards, target_hits):
    ...
    axes[0].plot(np.convolve(rewards, np.ones(window)/window, mode="valid"), color="steelblue", label=f"Moving avg ({window}ep)")
    axes[0].set_xlabel("Episode"); axes[0].set_ylabel("Cumulative Reward")
    axes[0].set_title("DQN Training - Reward Curve")
    axes[0].legend(); axes[0].grid(True, alpha=0.3)

    axes[1].plot(target_hits, alpha=0.3, color="green")
    axes[1].plot(np.convolve(target_hits, np.ones(window)/window, mode="valid"), color="green", label=f"Moving avg ({window}ep)")
    axes[1].set_xlabel("Episode"); axes[1].set_ylabel(f"Steps in [{TARGET_LOW}~{TARGET_HIGH}] Debye")
    axes[1].set_title("DQN Training - Target Hit Count")
    axes[1].legend(); axes[1].grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig("results/training_metrics.png", dpi=150)
    plt.close()
    print("[INFO] Training plots saved: results/training_metrics.png")
...
def main():
    ...
    print("[INFO] Model saved: models/dqn_molecule_model.pth")
